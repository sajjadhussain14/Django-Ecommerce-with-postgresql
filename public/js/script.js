function addToCart(product_id, csrfToken) {
    ...
    $.ajax({
      ...
      success: function (response) {
        setTimeout(() => {
          ...
          document.getElementById("pop-quantity").innerHTML =
            response.product.quantity;
          document.getElementById("pop-img").src = response.product.image_url;
          document.getElementById("pop-cart_total_amount").innerHTML =
            response.cart_total_amount;
          document.getElementById("pop-cart_total_qty").innerHTML =
            response.total_qty;
          ...
          for (var key in response.product.attributes) {
            if (response.product.attributes.hasOwnProperty(key)) {
              ...
              li.innerHTML =
                "<span>" +
                key +
                ':</span><span class="badge badge-primary badge-pill text-dark">' +
                response.product.attributes[key] +
                "</span>";
  ...
  const toggleCartAndWishlist = (e, csrfToken) => {
    ...
    try {
      selectedAttributes = document.getElementById(
        product_id + "-wish-attr-values",
      ).value;
    } catch (err) {
      console.log(err);
    }

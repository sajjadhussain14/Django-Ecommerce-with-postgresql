...
$(document).ready(function () {
  ...
  function updateURLAndRefresh() {
    ...
    let updatedURL = queryStringParts.length > 0 ? pathWithoutQuery + "?" + queryStringParts.join("&") : pathWithoutQuery;
...
const removeFilter = (key, val) => {
  ...
  // Construct the updated URL
  const updatedURL = queryStringParts.length > 0 ? pathWithoutQuery + "?" + queryStringParts.join("&") : pathWithoutQuery;
  // Replace the URL without triggering a page refresh
  window.history.replaceState({}, document.title, updatedURL);
  // Refresh the page to load data based on the updated filters
  window.location.reload();
